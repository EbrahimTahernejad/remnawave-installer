...
# ---- per-port install dir ----
NODE_DIR="/etc/remnanode-${NODE_PORT}"
...
echo "[2/3] Creating ${NODE_DIR} folder..."
mkdir -p "${NODE_DIR}"
# ---- YAML safe escaping (important) ----
ESCAPED_SECRET_KEY=$(printf "%s" "$SECRET_KEY" | sed "s/'/''/g")
cat > "${NODE_DIR}/docker-compose.yml" <<EOF
name: remnanode-${NODE_PORT}
services:
  remnanode:
    container_name: remnanode-${NODE_PORT}
    hostname: remnanode-${NODE_PORT}
